...
if not root_category.exists():
    log("KLAIDA: Pagrindinė kategorija nerasta!", level='error')
else:
    ...
    report_lines = ["=== TOP-DOWN LOGIKA IR ĮRAŠYMAS Į x_studio_mandatory_attributes ==="]

    for category in sorted_categories:
        # Nustatome protėvius, kad išvengtume dubliavimo (Paveldėjimo logika)
        ancestor_ids = []
        ...
        potential_attrs = {} # {Atributo_ID: Kiekis}
        attr_obj_map = {}   # {Vardas: Objektas} - kad vėliau nereiktų vėl searchinti
        ...
        for p in all_prods_in_branch:
            for line in p.attribute_line_ids:
                ...
                if attr.name not in already_used_above:
                    ...
                    attr_obj_map[attr.name] = attr

        universal_attr_ids = []
        
        # Tikriname universalumą (Veto principas)
        for attr_id, count in potential_attrs.items():
            attr_name = [name for name, obj in attr_obj_map.items() if obj.id == attr_id][0]
            is_universal = True
        ...
        # Rūšiuojame pagal populiarumą ir imam Top 5
        universal_attr_ids.sort(key=lambda x: x[2], reverse=True)
        top_5_data = universal_attr_ids[:5]

        if top_5_data:
            # --- ODOO M2M ĮRAŠYMAS ---
            # 1. Paruošiame ID sąrašą
            attr_to_write = [data[0] for data in top_5_data]
            # 2. Naudojame Odoo Command (6, 0, [ids]) - tai pakeičia esamus ryšius naujais
            category.write({
                'x_studio_mandatory_attributes': [(6, 0, attr_to_write)]
            })
            
            # Įrašome į atmintį vaikams, kad šie atributai jau "panaudoti" aukščiau
            category_assigned_names[category.id] = [data[1] for data in top_5_data]
            
            # Logas
            report_lines.append(f"\nKATEGORIJA: {category.display_name}")
            report_lines.append(f"  [+] Įrašyti atributai: {', '.join([d[1] for d in top_5_data])}")
